...
all_tools=open('../all_tools_stats_0.001.csv').read().strip().split('\n')[1:]           
for t in all_tools:
    t=t.split(',')
...
with open('../all_tools_CAMI2_TP_FP_0.001.csv', 'w') as outf:
    outf.write('tool,sample,rank,TP,FP\n')
    for t in sens_prec_dict:
        for s in sens_prec_dict[t]:
            for r in sens_prec_dict[t][s]:
                outf.write('{0},{1},{2},{3},{4}\n'.format(t,s,r,
                                                          sens_prec_dict[t][s][r]['TP'],
                                                          sens_prec_dict[t][s][r]['FP']))