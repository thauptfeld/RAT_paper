...
def do_rarefaction(input_file, list_of_percentages, iterations, path_to_out,profile):
    ...
    for p in list_of_percentages:
        ...
        for i in sampled_dict:
            ...
            # out1='{0}.rat_r.rf{1}.it{2}.csv'.format(smp, p, i)
            out2='{0}.rat_robust.rf{1}.min{2}_new.it{3}.csv'.format(smp, p, '0.001', i)
...
if __name__=='__main__':
    ...
    #             'CAMI_II_mousegut_w_dm/wbin/{0}_clean.complete.abundance'
    #             '.txt'.format(sample))
    
    input_file=('/hosts/linuxhome/phage/mgx/people/tina/RAT/benchmark/'
                'CAMI_II_mousegut/the10/{0}.RAT.read2classification.txt'.format(sample))
    profile=('/hosts/linuxhome/phage/mgx/people/tina/RAT/benchmark/'
             'CAMI_II_mousegut/abundance_files/{0}.RAT.complete.abundance.txt'.format(sample))
