...
def parse_arguments():
    ...
    if 'm' in args.mode:
        if not args.bin_folder:
            sys.exit('"m" was supplied to mode but no bin_folder. If you want '
                     'to include MAGs in your profile, please submit a bin '
                     'folder (and suffix if necessary)')
    ...
    if 'm' in args.mode:
        ...
        if args.bin_folder and args.bin2classification:
            ...
            # If CAT was run, use the CAT output files
                ...
                shared.give_user_feedback(message, args.log_file, args.quiet,
                    show_time=True)
                CAT_protein_fasta=('{0}.CAT.predicted_proteins.fasta'
                                   ''.format(args.out_prefix))
    ...
    for mapping in proc.stdout:
        ...
        # store unmapped reads as forward and reverse:
            ...
            if paired:
                ...
                unmapped_reads['fw'].add(read_id)
    print(paired)
    print(len(unmapped_reads['fw']))
    print(len(unmapped_reads['rev']))
