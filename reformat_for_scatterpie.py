...
with open('../revision/evaluations/20231020.CAMI2_marine.csv') as inf:
    for line in inf:
...
with open('../revision/evaluations/20231024.CAMI2_marine_scatterpie.csv', 'w') as outf:
...
with open('../revision/evaluations/20231020_CAMI2_plant.csv') as inf:
    for line in inf:
        correct, rank, value, tool, sample=line.strip().split(',')
        if sample not in data_dict:
            data_dict[sample]={}
...
with open('../revision/evaluations/20231024.CAMI2_plant_scatterpie.csv', 'w') as outf:
    outf.write('tool,rank,sample,TPR,A,B,C,D\n')
    for sample in data_dict:
        for rank in data_dict[sample]:
            for tool in data_dict[sample][rank]:
